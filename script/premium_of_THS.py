...
def get_data():
    ...
    # 请求同花顺
    result = json.loads(THS_RealtimeQuotes(str_code, 'latest;changeRatio;amount;', 'pricetype:2', True))["tables"]
    r = {}
    for i in range(len(result)):
        ...
        _amount = result[i]["table"]["amount"][0]
        r[_code] = {"time": _time, "latest": _latest, "changeRatio": _changeRatio, "amount": _amount}
    return r
...
def get_premium_rt():
    ...
    for k, v in r.items():
        tail = k.split(".")[1]
        # skdl1478
        if datetime.date(2020, 12, 9) == datetime.date.today():
            break
        #
        if k.strip(".SH").strip(".SZ") not in my_dict.keys():
            ...
            bond_amount = r[k]["amount"]
            stock_code = my_dict[k.strip(".SH").strip(".SZ")]["stock_code"]
            stock_amount = r[stock_code + "." + tail]["amount"]
            ...
            result_list.append(
                {"bond_price": bond_price, "bond_code": k, "bond_name": bond_name, "premium_rate": premium_rate,
                 ...
                 "bond_amount": bond_amount, "stock_amount": stock_amount})
    return result_list
