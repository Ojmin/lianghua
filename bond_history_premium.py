...
class Draw(object):
    ...
    def get_x_y(self, code, trans_price):
        ...
        x_time = []
        y_value = []
        for bar in stock_history_data:
            x_time_ = pds.to_datetime(bar["eob"]).tz_localize(None)
            bond_price = bond_history_data[bond_history_data["交易时间"] == x_time_]["收盘价"]
            y_value_ = bar["close"]
    ...
    def run(self):
        ...
        plt.legend()
        plt.savefig('my_picture/{}.png'.format(self.start_time.strftime('%Y-%m-%d %H:%M:%S').split(" ")[0]),dpi=500,bbox_inches = 'tight')
        plt.show()
